...
convertBtn.addEventListener('click', function () {
    if (input.value === '') {
        ...
        if (dropBtn.textContent === 'Fahrenheit (ºF)') {
            output.textContent = `Converted Temprature is : ${Math.round((Number(input.value) - 32) * (5 / 9))}ºC`
        } 
        else if (dropBtn.textContent === 'Celcius (ºC)') {
            output.textContent = `Converted Temprature is : ${Math.round((Number(input.value) * 9 / 5) + 32)}ºF`
        }
    }
